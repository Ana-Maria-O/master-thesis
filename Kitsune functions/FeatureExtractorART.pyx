...
class FEA:
    def __init__(self,file_path,limit=np.inf):
        self.path = file_path # Can be an ndarray if the input comes from ART
        self.limit = limit
    ...
    def __prep__(self):
        # If file is a path, proceed as normal
        ...
        # If path is an ndarray, set parse_type to the right value
        elif isinstance(self.path, np.ndarray):
            self.parse_type = "array"
        
        # If the path is not a file path or an ndarray, raise an exception
        else:
            print("The given input is not a filepath nor an ndarray.")
            raise Exception()
    ...
    def get_next_vector(self):
        ...
        ### Parse next packet ###
        if self.parse_type == "tsv" or self.parse_type == "array":
            if self.parse_type == "tsv":
                row = self.tsvin.__next__()
            else:
                row = self.path[self.curPacketIndx]
    ...
    # For ART: Add a packet to the list of packets
